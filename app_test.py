...
def gerar_pdf(conteudo_html):
    with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmpfile:
        c = canvas.Canvas(tmpfile.name, pagesize=letter)
        # Dividir o conteúdo HTML em linhas para desenhar no PDF
        lines = conteudo_html.split('\n')
...
def enviar_email(sender_email, password, destinatario, cc, assunto, corpo, pdf_path):
    ...
    with open(pdf_path, "rb") as f:
        ...
        attach.add_header('Content-Disposition', 'attachment', filename='proposta.pdf')
...
if 'conteudo_html' in st.session_state:
    ...
    with st.form("form_envio_email"):
        ...
        if enviar:
            enviar_email(sender_email, password, destinatario, cc, assunto, conteudo_html, pdf_path)
